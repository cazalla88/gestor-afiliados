        ...
        <article className={styles.articleContainer}>
                        ...
                        <div className="hub-hero-container" style={{ maxWidth: '1200px', margin: '2rem auto 4rem auto' }}>
                            ...
                            >
                                <img
                                    ...
                                    alt={SafeRender(campaign.title) || "Cover Image"}
            ...
            <div className={`container ${styles.contentGrid}`}>
                <div className={styles.mainContent}>
                    ...
                    {!isHub && content.pros && content.pros.length > 0 && (
                        <div style={{
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '0.5rem' }}>
                                {content.pros.slice(0, 3).map((pro: any, idx: number) => (
                                    <span key={idx} style={{
                                        background: 'white',
                    ...
                    {!isHub && (
                        <section id="pros-cons" className={styles.prosCons}>
                            <div className={styles.pros}>
                                <h3>{t.pros}</h3>
                                <ul>
                                    {content.pros?.map((p: any, i: number) => <li key={i}>{SafeRender(p)}</li>) || <li>Great Product</li>}
                                </ul>
                            </div>
                            <div className={styles.cons}>
                                <h3>{t.cons}</h3>
                                <ul>
                                    {content.cons?.map((c: any, i: number) => <li key={i}>{SafeRender(c)}</li>) || <li>None observed</li>}
                                </ul>
                            </div>
                        </section>
                    )}
                        ...
                        <section id="comparison" className={styles.comparison}>
                            ...
                            <div className={styles.tableWrapper}>
                                <table className={styles.table}>
                                    ...
                                    <tbody>
                                        {content.comparisonTable.map((row: any, i: number) => (
                                            <tr key={i} className={row.name.includes(campaign.productName) || i === 0 ? styles.highlightRow : ''}>
                                                <td>{SafeRender(row.name)}</td>
                                                <td>{SafeRender(row.price)}</td>
                                                <td>{SafeRender(row.rating)}</td>
                                                <td>{SafeRender(row.features)}</td>
                                                <td>
                                                    <a href={campaign.affiliateLink} target="_blank" rel="noopener noreferrer" className={styles.tableBtn}>{t.checkPrice}</a>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                    ...
                    <section id="verdict" className={styles.verdict}>
                        ...
                        {content.verdict ? (
                            // Use dangerouslySetInnerHTML to allow HTML in verdict (bolding, etc)
                            <div dangerouslySetInnerHTML={{ __html: content.verdict }} />
                        ) : (
                            <p>Great product overall.</p>
                        )}
                        {!isHub && (
                            <a href={campaign.affiliateLink} target="_blank" rel="noopener noreferrer" className={styles.buyBtnLarge}>
                                {t.buyNow}
                            </a>
                        )}
                    </section>
                    ...
                    <div className={styles.toc}>
                        ...
                        <nav>
                            <ul style={{ listStyle: 'none', padding: 0 }}>
                                <li><a href="#intro" style={{ color: '#555', textDecoration: 'none', fontSize: '0.95rem' }}>1. {t.whoFor}</a></li>

                                {/* Dynamic TOC from Parsed Features */}
                                {isHub ? (
                                    // HUB TOC: Direct list of sections
                                    featureHeaders.map((h, i) => (
                                        <li key={i}>
                                            <a href={`#${h.id}`} style={{ color: '#555', textDecoration: 'none', fontSize: '0.95rem' }}>{i + 2}. {SafeRender(h.text)}</a>
                                        </li>
                                    ))
                                ) : (
                                    ...
                                    <li>
                                        ...
                                        {featureHeaders.length > 0 && (
                                            <ul style={{ listStyle: 'none', paddingLeft: '1rem', marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                                {featureHeaders.map((h, i) => (
                                                    <li key={i}>
                                                        <a href={`#${h.id}`} style={{ color: '#888', textDecoration: 'none', fontSize: '0.85rem' }}>• {SafeRender(h.text)}</a>
                                                    </li>
                                                ))}
                                            </ul>
                                        )}
            ...
            {!isHub && (
                <StickyBar
                    ...
                    price={content.comparisonTable?.[0]?.price ? SafeRender(content.comparisonTable?.[0]?.price) : "€€€"}
